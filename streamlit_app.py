...
with col_main:
    ...
    with tab_chat:
        ...
        if st.session_state.get("disambiguation_pending"):
            disambiguation_data = st.session_state.disambiguation_pending
            funds = disambiguation_data["funds"]
            family = disambiguation_data["family"]
            
            with st.chat_message("assistant"):
                st.markdown(f"I found multiple funds under **{family.title()}**. Please select the specific fund you're interested in:")
            
            selected_fund = st.selectbox("Select a fund:", funds, key="disambig_selectbox")
        ...
        if prompt := st.chat_input("Ask about mutual funds..."):
            ...
            with st.chat_message("assistant"):
                ...
                matched_family = None
                for family, funds in fund_families.items():
                    if family in prompt_lower and len(funds) > 1:
                        # Check if the query doesn't already specify the exact fund name
                        has_specific_fund = any(fund.lower() in prompt_lower for fund in funds)
                        if not has_specific_fund:
